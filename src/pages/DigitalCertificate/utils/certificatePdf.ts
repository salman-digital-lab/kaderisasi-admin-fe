...
import {
  ...
  SCORE_SHEET_WIDTH,
  SCORE_SHEET_HEIGHT,
} from "../components/CertificateScoreSheet";
import type {
  CertificateElement,
  CertificateTemplate as CertificateTemplateData,
} from "../types";
...
export function salmanCertificateOverflow(source: HTMLElement): string[] {
  return Array.from(
...
export function certificatePdfErrorMessage(error: unknown): string | null {
  ...
  if (error.message.startsWith("CERTIFICATE_TEXT_OVERFLOW:")) {
    return `Bagian sertifikat melebihi halaman: ${error.message.slice("CERTIFICATE_TEXT_OVERFLOW:".length)}. Perbaiki desain lalu coba lagi.`;
  }
  if (error.message.startsWith("CERTIFICATE_SCORE_OVERFLOW:")) {
    return `Daftar nilai melebihi halaman kedua: ${error.message.slice("CERTIFICATE_SCORE_OVERFLOW:".length)}. Ringkas isi sebelum mengunduh.`;
  }
  return null;
}
...
async function renderCertificatePage(
  ...
  try {
    ...
    if (template.scoreSheetLayout === "salman-v1") {
      ...
      if (clipped.length)
        throw new Error(`CERTIFICATE_TEXT_OVERFLOW:${clipped.join(",")}`);
    }
